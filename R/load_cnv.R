...
aberration_kinds <- sort(unique(segmentation_matrix[,6]))+1;

# Detect the samples
samples <- unique(segmentation_matrix[,1]);
region_list <- list();
...
for(k in 1:length(aberration_kinds)){
	...
	ab_ids <- which(segmentation_matrix[,6]==(aberration_kinds[k]-1));
...
message("\nDone");
names(region_list) <- aberration_kinds-1;
return(region_list);
